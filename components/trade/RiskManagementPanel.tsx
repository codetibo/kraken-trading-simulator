...
import { ChevronDown, ChevronUp, AlertTriangle, TrendingUp, DollarSign, Target, Percent, GanttChart, Zap } from 'lucide-react';
import { convertSimpleTriggerToPrice, simpleTriggerTypeLabel, type SimpleTriggerType } from '@/lib/engine/triggerConversion';
...
const MODE_TABS: { id: TpSlMode; label: string; icon: React.ReactNode }[] = [
  ...
  { id: 'ADVANCED', label: 'Advanced', icon: <GanttChart className='h-3 w-3' /> },
];
...
function FieldLabel({ children, className }: { children: React.ReactNode; className?: string }) {
  return <label className={cn('mb-1 block text-[11px] font-medium text-muted-foreground', className)}>{children}</label>;
}
...
function PriceModeContent({
  stopLossPrice, onStopLossPriceChange,
  takeProfitPrice, onTakeProfitPriceChange,
  ...
  stopLossPrice: string; onStopLossPriceChange?: (v: string) => void;
  takeProfitPrice: string; onTakeProfitPriceChange?: (v: string) => void;
  trailingStopEnabled: boolean; onTrailingStopEnabledChange?: (v: boolean) => void;
  trailingOffsetType: 'PERCENT' | 'FIXED'; onTrailingOffsetTypeChange?: (v: 'PERCENT' | 'FIXED') => void;
  trailingOffsetValue: string; onTrailingOffsetValueChange?: (v: string) => void;
  trailingLimitOffset: string; onTrailingLimitOffsetChange?: (v: string) => void;
}) {
  return (
    <>
      ...
      <div>
        <div className='mb-1.5 flex items-center justify-between'>
          <label className='text-[11px] font-medium text-muted-foreground'>Trailing Stop</label>
          <button
            type='button'
          ...
          >
            <span
              className={cn(
                ...
                trailingStopEnabled ? 'translate-x-[18px]' : 'translate-x-[2px]',
              )}
            />
          </button>
        </div>
          ...
          <div className='space-y-2 rounded-md border border-border/50 bg-background p-2'>
            ...
            <div>
              ...
              <NumericInput
                ...
                placeholder={trailingOffsetType === 'PERCENT' ? '2.0' : '500.00'}
            ...
            <div>
              <div className='mb-1 flex items-center justify-between'>
                ...
                <span className='text-[10px] text-muted-foreground'>→ Trailing Stop Limit</span>
              </div>
              <NumericInput
...
function SimpleModeContent({
  ...
  const tpPrice = useMemo(
    () => convertSimpleTriggerToPrice(simpleTpType, tpVal, entryPrice, quantity, posSide, true),
    [simpleTpType, tpVal, entryPrice, quantity, posSide],
  );
  const slPrice = useMemo(
    () => convertSimpleTriggerToPrice(simpleSlType, slVal, entryPrice, quantity, posSide, false),
    ...
    <>
      ...
      <div className='rounded-md border border-border/50 p-2'>
        <div className='mb-1.5 flex items-center gap-1.5'>
          ...
          <span className='text-[11px] font-semibold uppercase tracking-wider text-positive'>Take Profit</span>
        </div>
        <div className='mb-1.5'>
          <FieldLabel>Trigger Type</FieldLabel>
          <TriggerTypeSelector value={simpleTpType} onChange={(v) => onSimpleTpTypeChange?.(v)} />
        </div>
        <div>
          <FieldLabel>
            Value {simpleTpType === 'PERCENT' ? '(%)' : simpleTpType === 'PNL_USD' ? '(USD)' : '(USD)'}
          </FieldLabel>
          <NumericInput
            value={simpleTpValue}
        ...
        {tpPrice > 0 && (
          <p className='mt-1 text-[10px] text-muted-foreground'>
            → Trigger at <span className='font-mono tabular-nums text-positive'>{formatUsd(tpPrice)}</span>
          </p>
        )}
      </div>
      ...
      <div className='rounded-md border border-border/50 p-2'>
        <div className='mb-1.5 flex items-center gap-1.5'>
          ...
          <span className='text-[11px] font-semibold uppercase tracking-wider text-negative'>Stop Loss</span>
        </div>
        <div className='mb-1.5'>
          <FieldLabel>Trigger Type</FieldLabel>
          <TriggerTypeSelector value={simpleSlType} onChange={(v) => onSimpleSlTypeChange?.(v)} />
        </div>
        <div>
          <FieldLabel>
            Value {simpleSlType === 'PERCENT' ? '(%)' : simpleSlType === 'PNL_USD' ? '(USD)' : '(USD)'}
          </FieldLabel>
          <NumericInput
            value={simpleSlValue}
        ...
        {slPrice > 0 && (
          <p className='mt-1 text-[10px] text-muted-foreground'>
            → Trigger at <span className='font-mono tabular-nums text-negative'>{formatUsd(slPrice)}</span>
          </p>
        )}
      </div>
...
function AdvancedModeContent({
  ...
  return (
    <div className='space-y-3'>
      ...
      <div className='rounded-md border border-border/50 p-2'>
        <div className='mb-1.5 flex items-center gap-1.5'>
          <span className='flex h-4 w-4 items-center justify-center rounded-full bg-positive/20 text-[9px] font-bold text-positive'>1</span>
          <span className='text-[11px] font-semibold uppercase tracking-wider text-positive'>Profit-Taking</span>
        </div>
        <SegmentedSelect
          value={advField1}
        ...
        {advField1 !== 'LIMIT' && (
          <div className='mt-2 space-y-2'>
            ...
            <div>
              <FieldLabel>
                {advField2Type === 'DISTANCE' ? 'Distance from Entry' : 'Target P&L (USD)'}
              </FieldLabel>
              <div className='relative'>
                <input
                  ...
                  placeholder={advField2Type === 'DISTANCE' ? '1000 (or 5% for percent)' : '5000'}
                  step='any'
      ...
      <div className='rounded-md border border-border/50 p-2'>
        <div className='mb-1.5 flex items-center gap-1.5'>
          <span className='flex h-4 w-4 items-center justify-center rounded-full bg-negative/20 text-[9px] font-bold text-negative'>3</span>
          <span className='text-[11px] font-semibold uppercase tracking-wider text-negative'>Stop-Loss</span>
        </div>
        <SegmentedSelect
          value={advField3}
        ...
        <div className='mt-2 space-y-2'>
          <div>
            <FieldLabel>
              {advField3 === 'TRAILING_STOP' || advField3 === 'TRAILING_STOP_LIMIT'
          ...
          {(advField3 === 'STOP_LOSS_LIMIT_DISTANCE' || advField3 === 'STOP_LOSS_LIMIT_PNL' || advField3 === 'TRAILING_STOP_LIMIT') && (
            <div>
              <FieldLabel>
                {advField3 === 'TRAILING_STOP_LIMIT' ? 'Limit Offset' : 'Limit Buffer'}
              </FieldLabel>
              <input
                type='number'
...
function AccountRiskSection({
  ...
  const suggestedMargin = marketType === 'MARGIN' ? suggestedNotional / leverage : 0;
      ...
      <div className='rounded-md bg-accent/20 p-2'>
        ...
        <div className='space-y-0.5'>
          ...
          <div className='flex items-center justify-between'>
            ...
            <span className='font-mono text-[11px] tabular-nums text-foreground'>{formatUsd(suggestedNotional)}</span>
          </div>
          {marketType === 'MARGIN' && (
            <div className='flex items-center justify-between'>
              <span className='text-[11px] text-muted-foreground'>Margin</span>
              <span className='font-mono text-[11px] tabular-nums text-foreground'>{formatUsd(suggestedMargin)}</span>
            </div>
          )}
        </div>
      </div>
...
export const RiskManagementPanel = memo(function RiskManagementPanel({
  ...
  const effectiveTpPrice = useMemo(
    () => (tpSlMode === 'SIMPLE' ? convertSimpleTriggerToPrice(simpleTpType, simpleTpVal, entry, qty, positionSide || 'LONG', true) : parseFloat(takeProfitPriceProp) || 0),
    [tpSlMode, simpleTpType, simpleTpVal, entry, qty, positionSide, takeProfitPriceProp],
  );
  const effectiveSlPrice = useMemo(
    () => (tpSlMode === 'SIMPLE' ? convertSimpleTriggerToPrice(simpleSlType, simpleSlVal, entry, qty, positionSide || 'LONG', false) : parseFloat(stopLossPriceProp) || 0),
    [tpSlMode, simpleSlType, simpleSlVal, entry, qty, positionSide, stopLossPriceProp],
  );
  ...
  const riskPercentOfNotional = notional === 0 ? 0 : (riskAmount / notional) * 100;
    ...
    <Card className='mb-3 overflow-hidden border-border/60'>
      ...
      >
        <div className='flex items-center gap-1.5'>
          ...
          <span className='text-[11px] font-medium uppercase tracking-wider text-muted-foreground'>Risk Management</span>
          {(hasValidLoss || hasValidReward) && (
            <span
              className={cn(
                'inline-flex h-4 items-center rounded-full px-1.5 text-[11px] font-medium',
                isGoodRatio ? 'bg-positive/10 text-positive' : 'bg-amber-500/10 text-amber-500',
              )}
            >
        ...
        <CardContent className='space-y-3 p-2.5 pt-1'>
              ...
              {[0.5, 1, 2, 5].map((pct) => (
                <button
                  ...
                  className={cn(
                    ...
                    riskPercent === pct ? 'bg-accent text-accent-foreground' : 'bg-transparent text-muted-foreground hover:text-foreground',
                  )}
                >
                  {pct}%
                </button>
          ...
          <div className='grid grid-cols-2 gap-x-3 gap-y-1.5'>
            <div className='flex items-center justify-between'>
              ...
              <span className={cn('font-mono text-[11px] tabular-nums', hasValidLoss ? 'text-negative' : 'text-muted-foreground')}>
            ...
            <div className='flex items-center justify-between'>
              ...
              <span className={cn('font-mono text-[11px] tabular-nums', hasValidReward ? 'text-positive' : 'text-muted-foreground')}>
            ...
            <div className='flex items-center justify-between'>
              ...
              <span
                className={cn(
                  ...
                  riskRewardRatio > 0 ? (isGoodRatio ? 'text-positive' : 'text-amber-500') : 'text-muted-foreground',
                )}
              >
            ...
            <div className='flex items-center justify-between'>
              ...
              <span className={cn('font-mono text-[11px] tabular-nums', hasValidLoss ? 'text-negative' : 'text-muted-foreground')}>
                {hasValidLoss ? formatPercent(riskPercentOfNotional, { signed: false }) : '—'}
              </span>
            </div>
          </div>
          ...
          {hasValidLoss && hasValidReward && !isGoodRatio && riskRewardRatio > 0 && (
            <p className='flex items-center gap-1 text-[11px] text-amber-500'>
              <AlertTriangle className='h-3 w-3' />
              Low risk/reward ratio. Consider a wider take profit or tighter stop loss.
            </p>
          )}
        </CardContent>
      )}
