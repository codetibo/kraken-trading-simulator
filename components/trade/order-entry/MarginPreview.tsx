 ...
 *
 * Generated once at module load (outside of render) so the component stays
 * pure — calling `Date.now()` / `Math.random()` during render is forbidden by
 * React's rules of hooks/components. The data is mock/illustrative only, so a
 * single module-level snapshot is perfectly acceptable.
 */
function generateFundingHistory(): { time: number; rate: number }[] {
  const now = Date.now();
...
const FUNDING_HISTORY = generateFundingHistory();
...
export function MarginPreview({
  ...
  const takerFeeNotional =
    notional * (DEFAULT_FEE_SCHEDULE.takerFeeBps / 10000);
  const makerFeeNotional =
    notional * (DEFAULT_FEE_SCHEDULE.makerFeeBps / 10000);
  ...
  // ── Simulated funding rate history (last 12 × 4h periods = 48h) ──
  // Generated once at module load (see FUNDING_HISTORY) to keep render pure.
  const fundingHistory = FUNDING_HISTORY;

  const currentFundingRate =
    fundingHistory[fundingHistory.length - 1]?.rate ?? 0;
    ...
    marketType === 'MARGIN' ? (
      <Card className='mb-3'>
        <CardContent className='space-y-1 p-2.5'>
            ...
            value={
              <span
                className={cn(
                  ...
                  marginStatusColor(marginStatusPct),
                )}
              >
          ...
          <PreviewRow label='Liq. Price' value={formatUsd(liqPrice)} />
          <PreviewRow
            label='Liq. Distance'
            value={
              <span
          ...
          <PreviewRow
            ...
            value={
              <div className='text-right'>
                <span className='font-mono text-[11px] tabular-nums text-foreground'>
                  {formatUsd(marginFeePer24h)}
                </span>
                <br />
                <span className='text-[10px] text-muted-foreground'>
                  ({formatPercent(0.0001 * 6, { signed: false })} of notional)
                </span>
              </div>
            }
          />
            ...
            <div className='mt-0.5 flex justify-between text-[10px] text-muted-foreground'>
              <span>
                ...
                <span
                  className={cn(
                    'font-mono tabular-nums',
                    minFunding < 0 ? 'text-negative' : 'text-foreground',
                  )}
                >
                  {formatPercent(minFunding, { signed: true })}
                </span>
              </span>
              <span>
                Max:{' '}
                <span
                  className={cn(
                    'font-mono tabular-nums',
                    maxFunding > 0 ? 'text-positive' : 'text-foreground',
                  )}
                >
                  {formatPercent(maxFunding, { signed: true })}
                </span>
              </span>
            </div>
          </div>
